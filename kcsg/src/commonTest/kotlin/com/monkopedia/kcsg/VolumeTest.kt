...
class VolumeTest {
    ...
    fun vlumeTest() {
        run {
            ...
            assertEquals(emptyVolume, 0.0, 1e-16)
        }
        run {
        ...
        run {
            ...
            assertEquals(4.0 * PI * r * r * r / 3.0, volumeSphere1, 10.0)

            // better approximation
            val volumeSphere2 = Sphere(r, 256, 128).toCSG().computeVolume()
            assertEquals(4.0 * PI * r * r * r / 3.0, volumeSphere2, 5e-2)
        }
        run {
            ...
            assertEquals(PI * r * r * h, volumeCylinder1, 10.0)

            // better approximation
            val volumeCylinder2 = Cylinder(r, h, 256).toCSG().computeVolume()
            assertEquals(PI * r * r * h, volumeCylinder2, 5e-2)
        }
    }
}
