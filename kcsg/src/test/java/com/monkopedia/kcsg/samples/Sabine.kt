...
class Sabine {
    fun toCSG(): CSG {
        ...
        for (i in 0..49) {
            val angleX = 45 + random.nextDouble() * 90
            val angleZ = 45 + random.nextDouble() * 90
            var fracture1 = Cube(
                0.1 + fractW * random.nextDouble(),
                fractL * random.nextDouble(),
                fractD
            ).noCenter().toCSG().transformed(Transform.unity().rotZ(-angleZ).rotX(-angleX))
            val x = -w / 2.0 + random.nextDouble() * (w - 2)
            val y = -h / 2.0 + random.nextDouble() * (h - 2)
